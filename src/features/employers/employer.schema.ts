...
export const employerProfileSchema = z.object({
    ...
    description: z.string().trim().min(10 , "Description must be atleast 10 characters long")
    ...
    yearOfEstablishment :z.string().trim().regex(/^\d{4}$/, "Please enter a valid 4-digit year").refine((year) => {
...
location:z.string().trim().min(2,"Location must be at least 2 characters long").max(255,"Location must not exceed 255 characters").optional().or(z.literal("")),
